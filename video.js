...
let config = {
    appid:'5ee5c00ce46d43619ff71ddcddfd6d78',
    token:'68f8838e734245d6b3d319b8eb896aff',
    uid:null,
    channel:'Soharab',
}
...
let joinStreams = async () => {
    ...
    client.on("volume-indicator", function(evt){
        for (let i = 0; evt.length > i; i++){
            ...
            if(volume > 0){
                document.getElementById(`volume-${speaker}`).src = 'volume-on-copy.svg'
            }else{
    ...
    let player = `<div class="video-containers" id="video-wrapper-${config.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${config.uid}" src="volume-on-copy.svg" /> ${config.uid}</p>
...
let handleUserJoined = async (user, mediaType) => {
    ...
    if (mediaType === 'video'){
        ...
        player = `<div class="video-containers" id="video-wrapper-${user.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${user.uid}" src="volume-on-copy.svg" /> ${user.uid}</p>
